...
- Upgraded the minimum supported PHP CodeSniffer version to 4.0.
- Updated namespaced function, `Closure`, and first-class callable detection for
  PHPCS 4's unified name tokens, and removed obsolete JS-tokenizer handling.

## [0.5.5] - 2026-03-16

### Fixed

- Squiz.Commenting.BlockComment now globally suppressed via `severity=0` instead of `<exclude>`. The previous `<exclude>` only removed the sniff from our own rule reference — when another standard (e.g. WordPress-Extra) also included the Squiz sniff, both it and our replacement `Emrikol.Comments.BlockComment` would fire, producing duplicate errors.
